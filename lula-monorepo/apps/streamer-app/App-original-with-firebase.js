...
function AppContent() {
  ...
  useEffect(() => {
    if (user?.id) {
      console.log('🔌 Initializing WebSocket connection for streamer:', user.id);
      ...
      WebSocketService.on('commission-updated', (data) => {
        console.log('💎 Commission updated via WebSocket:', data);
      ...
      return () => {
        console.log('🔌 Disconnecting WebSocket for streamer:', user.id);
...
export default function App() {
  ...
  // Original complex app (commented out for testing)
  /*
  try {
    console.log('🔍 Creating providers...');
    return (
    ...
    console.error('❌ Error in main App component:', error);
    return <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}><Text>App Error: {error.message}</Text></View>;
  }
  */
}
